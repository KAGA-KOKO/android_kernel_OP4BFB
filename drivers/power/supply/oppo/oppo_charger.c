...
#ifdef ODM_WT_EDIT
/* Yize.Zhang@ODM_WT.BSP.Tp.Funtion, 20200413,Add for usb status notify tp funtion*/
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/4/7,distinguish charge ic vendor*/
...
char *aicr_voter[8] = {"usb/ac","batt_temp","lcd","camera","ap_temp","cpu_temp","phonecall","atm"};
enum aicl_by {
	CHARGER = 0,
	TBATT,
	LCD,
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@WT.CHG,2020/6/28,add for oppoversion*/
extern bool is_project_pascalE(void);
#endif /* ODM_WT_EDIT */
#ifdef  CONFIG_FB
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/14, kernel panic for NULL pointer*/
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oppo_usb_get_property:oppo_chg_chip is NULL\n");
			pr_err("oppo_usb_get_property: property %d\n", psp);
			ret = -EINVAL;
			val->intval = 0;
			return ret;
		}
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/14, kernel panic for NULL pointer*/
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oppo_usb_set_property: oppo_chg_chip is NULL\n");
			ret = -EINVAL;
			return ret;
		}
#endif /*ODM_WT_EDIT*/
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/14, kernel panic for NULL pointer*/
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oppo_ac_get_property: oppo_chg_chip is NULL\n");
			pr_err("oppo_ac_get_property: property %d\n", psp);
			ret = -EINVAL;
			val->intval = 0;
			return ret;
		}
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/14, kernel panic for NULL pointer*/
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oppo_batery_set_property: oppo_chg_chip is NULL\n");
			ret = -EINVAL;
			return ret;
		}
#endif /*ODM_WT_EDIT*/
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/14, kernel panic for NULL pointer*/
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oppo_battery_get_property: oppo_chg_chip is NULL\n");
			pr_err("oppo_battery_get_property: %d\n", psp);
			val->intval = 0;
			ret = -EINVAL;
			return ret;
		}
...
//Junbo.Guo@ODM_WT.BSP.CHG, 2020/04/10, Modify for fast node
	...
	if (chip->pascale_node && is_project_pascalE()) {
		node = chip->pascale_node;
		pr_info("%s pascale changed device_node\n", __func__);
	}
#endif
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/27, add for usb overtemp protection*/
	chip->usb_overtemp_protection = of_property_read_bool(node, "usb_overtemp_protection");
#endif
...
#ifdef ODM_WT_EDIT
//Sidong.Zhao@ODM_WT.BSP.CHG, 2019/12/2, implement stop charge
	   if ((chip->stop_chg == 0) && (chip->charger_type == POWER_SUPPLY_TYPE_USB || chip->charger_type == POWER_SUPPLY_TYPE_USB_CDP)&&(charging_current > 100)) {
...
#define BATT_TEMP_EXTEND_T1_TO_T2_ICHG_PART2 3000  //VBAT>4.18
#define BATT_TEMP_EXTEND_T2_TO_T3_ICHG 3000
#define BATT_TEMP_EXTEND_T3_TO_T4_ICHG 1600
#define BATT_TEMP_EXTEND_T4_TO_T5_ICHG 1600 //need vbus to 5v
#define BATT_TEMP_EXTEND_T4_TO_T5_ICHG2 1600
#define BATT_TEMP_EXTEND_T4_TO_T5_ICHG3 1600
...
#ifdef ODM_WT_EDIT
//Sidong.Zhao@ODM_WT.BSP.CHG, 2019/12/2, implement stop charge
	   if ((chip->stop_chg == 0) && (chip->charger_type == POWER_SUPPLY_TYPE_USB || chip->charger_type == POWER_SUPPLY_TYPE_USB_CDP)&&(current_limit > 100)) {
			current_limit = 100;
			voter = ATM;
        }
...
//Sidong.Zhao@WT.CHG,2020/6/29,add for disable highvolt charger when flahlights on
                if (val) {
                        oppo_chg_set_charger_vol_9v(g_charger_chip, false);
                } else {
                        oppo_chg_set_charger_vol_9v(g_charger_chip, true);
                }
#endif /* ODM_WT_EDIT*/
        }
}
...
#ifdef ODM_WT_EDIT
//Sidong.Zhao@CHG.WT,2020/6/17, new notification logic
...
#ifdef ODM_WT_EDIT
/* Sidong.Zhao@WT.CHG,2020/6.18, modify vbus threshold for shutdown condition*/
                if ((chip->chg_ops->check_chrdet_status() == false) && (chip->charger_volt < 3500)) {
#else
...
#ifdef ODM_WT_EDIT
//Sidong.Zhao@WT.CHG,2020/6/29,add for disable highvolt charger when flahlights on
	if (chip->camera_on && pdqc_9v == true) {
		oppo_chg_set_charger_vol_9v(chip, false);
	}
#endif /*ODM_WT_EDIT*/
}
static void oppo_chg_qc_config(struct oppo_chg_chip *chip)
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/4/7,distinguish charge ic vendor*/
int get_charger_ic_det(struct oppo_chg_chip *chip)
{
	int count = 0;
