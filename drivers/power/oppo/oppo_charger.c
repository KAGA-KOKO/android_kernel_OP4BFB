...
#ifdef ODM_WT_EDIT
static int resistance_id[] = {27000, 150000, -1};
#endif
		...
		enum power_supply_property psp)
{
	...
	switch (psp) {
#ifdef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add force temp func to debug*/
...
#ifndef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.29 19181 not support*/
#ifdef CONFIG_OPPO_SMART_CHARGER_SUPPORT
...
/* WT000695@ODM_WT.BSP.Charger.vendor.20191218, Add Battery MMI hardware info in KERNEL */
		...
		case POWER_SUPPLY_PROP_RESISTANCE_ID:
			val->intval = resistance_id[chip->batt_id];
			break;
#endif
#ifdef CONFIG_OPPO_CHARGER_MTK
		case POWER_SUPPLY_PROP_STOP_CHARGING_ENABLE:
...
#ifdef ODM_HQ_EDIT
/*zhouyi@ODM.HQ.BSP.CHG.Basic 2020.01.08 add to solve USB KE*/
bool oppo_chg_init_done = false;
#endif /*ODM_HQ_EDIT*/
...
#ifdef ODM_HQ_EDIT
/*xiangchao.zhong@BSP.CHG.thermal  when calling on, and temp higher 30, limit charging current to 0.5A*/
    rc = of_property_read_u32(node, "qcom,charge_current_calling_ma",
        &chip->limits.charge_current_calling_ma);
    if (rc) {
        chip->limits.charge_current_calling_ma = OPCHG_INPUT_CURRENT_LIMIT_CALLING_MA;
    }
#endif
	...
	rc = of_property_read_u32(node, "qcom,input_current_vooc_ma_led",
			&chip->limits.input_current_vooc_ma_led);
	if (rc) {
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			...
			chg_ctrl_by_lcd = %d, \
			chg_ctrl_by_camera = %d, \
			chg_ctrl_by_calling = %d, \
			input_current_calling_ma = %d, \
			charge_current_calling_ma = %d, \
			...
			chip->limits.input_current_charger_ma,
			chip->limits.input_current_usb_ma,
			chip->chg_ctrl_by_lcd,
			chip->chg_ctrl_by_camera,
			chip->chg_ctrl_by_calling,
			chip->limits.input_current_calling_ma,
			chip->limits.charge_current_calling_ma,
			chip->limits.input_current_led_ma,
			chip->limits.input_current_led_ma_normal,
...
#ifdef ODM_HQ_EDIT
/*xiangchao.zhong@BSP.CHG.thermal  when call, and temp higher 30, limit charging current to 0.5A*/
    if ((chip->chg_ctrl_by_calling) && (chip->calling_on)
              && (charging_current > chip->limits.charge_current_calling_ma)) {
        charging_current = chip->limits.charge_current_calling_ma;
        charger_xlog_printk(CHG_LOG_CRTI, 
                "[BATTERY][Charging Current]calling ON, temp = %d, current: %d\n", chip->temperature, charging_current);
    }
#endif
...
#endif
	/*WT000695@ODM.BSP.Charger 2020/03/03, Add check Vbus volt before turn on charging*/
	int chg_volt = chip->chg_ops->get_charger_volt();
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			if (oppo_vooc_get_fastchg_to_normal() == true
					|| oppo_vooc_get_fastchg_to_warm() == true
					/*WT000695@ODM.BSP.Charger 2020/03/03, Add check Vbus volt before turn on charging*/
					|| (chg_volt > chip->limits.charger_hv_thr)) {
				charger_xlog_printk(CHG_LOG_CRTI,
					"fast_to_normal or to_warm or charger_hv 1,don't turn on charge here\n");
			} else {
			...
			if (oppo_vooc_get_fastchg_to_normal() == true
				...
				/*do nothing*/
				charger_xlog_printk(CHG_LOG_CRTI,
					"fast_to_normal or to_warm or charger_hv 2,don't turn on charge here\n");
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	if (oppo_vooc_get_fastchg_started() == true) {
		...
		chip->batt_id = oppo_gauge_get_batt_id();
	}
...
{
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.07 for notify bugs */
	if (chip->vbatt_over == false && chip->batt_full == true && chip->charger_exist == true && chip->ui_soc != 100){
#else
...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.07 for notify bugs */
	if (chip->charger_exist && chip->batt_exist && chip->vbatt_over == false
#else
...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.07 for notify bugs */
	} else if (chip->charger_exist && chip->batt_exist && chip->vbatt_over == false
#else
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL)
		...
		if (chip->recharge_after_full == true || chip->vbatt_num == 2) {
			...
			if (chip->dod0_counts == DOD0_COUNTS) {
				...
				oppo_chg_voter_charging_start(chip, CHG_STOP_VOTER__FULL);
				charger_xlog_printk(CHG_LOG_CRTI,
					"oppo_chg_check_status_full,dod0_counts = %d\n",
					chip->dod0_counts);
			}
		}
...
void oppo_smart_charge_by_cool_down(struct oppo_chg_chip *chip, int val)
{
	...
	if (val & SMART_NORMAL_CHARGER_1500MA) {
		...
		chip->limits.input_current_vooc_ma_high = OPPO_CHG_1500_CHARGING_CURRENT;
		chip->limits.input_current_vooc_ma_warm = OPPO_CHG_1500_CHARGING_CURRENT;
		chip->limits.input_current_vooc_ma_normal = OPPO_CHG_1500_CHARGING_CURRENT;
